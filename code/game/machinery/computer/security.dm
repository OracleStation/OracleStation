...
//FIELD FUNCTIONS
			if("Edit Field")
				...
				switch(href_list["field"])
					...
					if("species")
						if(istype(active1, /datum/data/record))
							var/t1 = input("Select a species", "Species Selection") as null|anything in CONFIG_GET(keyed_number_list/roundstart_races)
							if(!canUseSecurityRecordsConsole(usr, t1, a1))
								return
	...
	for(var/datum/data/record/R in GLOB.data_core.security)
		if(prob(10/severity))
			switch(rand(1,8))
				if(1)
					if(prob(10))
						R.fields["name"] = "[pick(unathi_name(MALE),unathi_name(FEMALE))]"
					else
				...
				if(7)
					R.fields["species"] = pick(CONFIG_GET(keyed_number_list/roundstart_races))
				if(8)
