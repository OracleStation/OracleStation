...
/datum/admins/proc/check_antagonists()
	if (SSticker.HasRoundStarted())
		...
		if(CONFIG_GET(keyed_flag_list/continuous)[SSticker.mode.config_tag])
			dat += ", <a href='?_src_=holder;toggle_midround_antag=1'>[CONFIG_GET(keyed_flag_list/midround_antag)[SSticker.mode.config_tag] ? "creating replacement antagonists" : "not creating new antagonists"]</a><BR>"
		else
			dat += "<BR>"
