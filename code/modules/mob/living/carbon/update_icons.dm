...
/mob/living/carbon/update_transform(var/instant = FALSE)
	...
	if(changed)
		animate(src, transform = ntransform, time = (instant ? 0 : 2), pixel_y = final_pixel_y, dir = final_dir, easing = EASE_IN|EASE_OUT)
